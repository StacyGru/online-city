...
class MonitorsList extends React.Component {
    ...
    componentDidMount() {
        let data;
        axios.get('http://127.0.0.1:8000/monitor_list')
            .then(res => {
                data = res.data;
                this.setState({
                    details: data
            ...
            <div>
                <div className="grid grid-cols-3 grid-rows-1 px-1/10">
                    <p className="h-fit w-fit text-xs font-light text-mainGray">
                        ...
                        <span> > </span>
                        <Link to="/monitors" className="hover:underline">Мониторы</Link>
                    </p>
                    <h1 className="text-xl lg:text-3xl font-bold mb-10 text-center">Мониторы</h1>
